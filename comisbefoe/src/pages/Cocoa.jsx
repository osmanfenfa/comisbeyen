...
import { calculateMoistureDeductionPrice } from "../utils/pricing.js";
...
export default function Cocoa() {
  ...
  useEffect(() => {
    if (prices?.cocoa_price_per_kg && !pricePerKg) {
      setPricePerKg(prices.cocoa_price_per_kg.toString());
    }
  }, [prices]);
  ...
  const loadPendingPurchases = async () => {
    ...
    try {
      const { data } = await client.get("/cocoa/?status=pending");
      setPendingList(data);
    } catch (err) {
      console.error("Could not load pending cocoa transactions", err);
    } finally {
  ...
  const water = parseFloat(waterPercent) || 0;
  const price = parseFloat(pricePerKg) || 0;

  const calculation = (weight > 0 && water >= 0 && price > 0)
    ? calculateMoistureDeductionPrice(weight, water, standardPercent, price)
    : null;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!navigator.onLine) {
        await queueOfflinePurchase("cocoa", payload);
        setFormMessage({
          type: "success",
          text: "Offline: Purchase saved to local queue! It will sync when connected.",
        });
      } else {
        await client.post("/cocoa/", payload);
        setFormMessage({
          type: "success",
  ...
  const handleApprove = async (txnId) => {
    try {
      await client.post(`/cocoa/${txnId}/approve`);
      loadPendingPurchases();
    } catch (err) {
  ...
  const handleConfirmReject = async () => {
    ...
    try {
      await client.post(`/cocoa/${rejectModalTxn.id}/reject`, {
  ...
  const handleIssueReceipt = async (txn) => {
    try {
      ...
      const { data } = await client.post(`/cocoa/${txn.id}/issue-receipt?loan_deduction=${deduction}`);
    ...
    <div className="p-4 space-y-5 pb-24">
      ...
      <div>
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold text-slate-900">Cocoa Purchasing</h1>
        </div>
        <p className="text-xs text-slate-500 mt-0.5">
          Moisture-deduction pricing · Standard allowance: {standardPercent}%
        </p>
      </div>
      ...
      {rejectModalTxn && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-xs">
          <div className="bg-white w-full max-w-sm rounded-2xl p-5 space-y-4 shadow-xl">
            <h3 className="font-bold text-slate-900 text-sm">Reject Cocoa Transaction</h3>
