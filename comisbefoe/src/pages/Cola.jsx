...
export default function Cola() {
  ...
  useEffect(() => {
    if (prices?.cola_price_per_kg && !pricePerKg) {
      setPricePerKg(prices.cola_price_per_kg.toString());
    }
  }, [prices]);
  ...
  const loadPendingPurchases = async () => {
    ...
    try {
      const { data } = await client.get("/cola/?status=pending");
      setPendingList(data);
    } catch (err) {
      console.error("Could not load pending cola transactions", err);
    } finally {
  ...
  const handleApprove = async (txnId) => {
    try {
      await client.post(`/cola/${txnId}/approve`);
      loadPendingPurchases();
    } catch (err) {
  ...
  const handleConfirmReject = async () => {
    ...
    try {
      await client.post(`/cola/${rejectModalTxn.id}/reject`, {
  ...
  const handleIssueReceipt = async (txn) => {
    try {
      ...
      const { data } = await client.post(`/cola/${txn.id}/issue-receipt?loan_deduction=${deduction}`);
    ...
    <div className="p-4 space-y-5 pb-24">
      ...
      <div>
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold text-slate-900">Cola Nut Purchasing</h1>
        </div>
      ...
      {rejectModalTxn && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-xs">
          <div className="bg-white w-full max-w-sm rounded-2xl p-5 space-y-4 shadow-xl">
            <h3 className="font-bold text-slate-900 text-sm">Reject Cola Nut Transaction</h3>
